...
int main(int argc, char *argv[]) {
  ...
  int serverPort, sd, rc, n, echoLen, flags, error, timeOut, fd, seqnum, totalseq, rcvdNumber, expected = 0, randomPassed, randomlyGenerated;
  struct sockaddr_in cliAddr, remoteServAddr, echoServAddr;
  struct hostent *h;
  char msg[MAX_MSG], *msgptr, ack[2];


  /* check command line args */
  if(argc<5) {
    printf("usage : %s <sender_hostname> <sender_portnumber> <filename> <lossfrequency>\n", argv[0]);
    exit(1);
  }
  ...
  randomPassed = atof(argv[4]) * RAND_MAX;
  srand(time(NULL));
  ...
  //  }
    ...
    printf("Sending ACK %d... ", (seqnum+1)%2);
    if(randomlyGenerated > randomPassed || seqnum == totalseq) { // last ACK always succeeds (typically would be resolved with connection termination protocol)
      sendto(sd, ack, strlen(ack)+1, flags,
      (struct sockaddr *) &remoteServAddr,
      sizeof(remoteServAddr));
      printf("done\n");
    } else {
